...
namespace BAG.admin
{
    public partial class article : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (Request.Params["deleteArticle"] != null)
                    deleteArticle();                
            }
            ...
            try
            {
                ...
                if (lis.Count > 0)
                {
                    ...
                    for (int i = 0; i < lis.Count; i++)
                    {
                        ...
                        span.Attributes.Add("onclick", "bagarticle.deleteArticle('" + lis[i].ElementAt(0).Value + "')");
                        ...
                        span.Attributes.Add("href", "editArticle.aspx?article=" + lis[i].ElementAt(0).Value);
        ...
        void deleteArticle()
        {
            try
            {
                ...
                int i = 0;
                int id = Int32.TryParse(Request.Params["deleteArticle"], out i) ? i : 0;
                if (id != 0)
                {
                    bagClass b = new bagClass();
                    b.connect("deleteArticle", new string[] { id.ToString() });
                }
            }
            catch (Exception er)
            {
